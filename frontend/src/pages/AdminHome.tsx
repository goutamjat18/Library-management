...
import { Wrench, BarChart3, ArrowLeftRight } from 'lucide-react';
...
const menuItems = [
  { icon: Wrench, label: 'Maintenance', route: '/maintenance', desc: 'Manage books, memberships & users' },
  { icon: BarChart3, label: 'Reports', route: '/reports', desc: 'View analytics & data reports' },
  { icon: ArrowLeftRight, label: 'Transactions', route: '/transactions', desc: 'Issue, return & manage books' },
];
  ...
  return (
    <PageLayout title="Admin Dashboard">
      <div className="glass-card p-6 mb-6">
        ...
        <p className="text-sm text-muted-foreground mb-5">Manage your library system</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {menuItems.map((item) => (
            <button
              key={item.label}
              onClick={() => navigate(item.route)}
              className="group relative overflow-hidden rounded-xl p-5 text-left transition-all duration-200 hover:-translate-y-0.5"
